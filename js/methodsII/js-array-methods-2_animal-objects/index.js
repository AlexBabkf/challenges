...
const firstAnimalStartingWithLetterG = animals.find((dict) =>
  dict.name.startsWith("g")
);

const indexOfAnimalWithNameLongerFive = animals.findIndex(
  (dict) => dict.name.length > 5
);
...
const animalsSortedByWeightReversed =
  animalsSortedByWeightStartingWithLowest.slice().reverse();

const animalWithWeightMoreThanFivehundredExists = animals.some(
  (dict) => dict.weight > 500
);
...
const allAnimalsInEuropeWeighLessThanOnehundred = animals
  .filter((dict) => dict.continents.includes("Europe"))
  .every((dict) => dict.weight < 100);
// Hint: filter + map + reduce
const weightOfAllAnimalsInAfrica = animals
  .filter((dict) => dict.continents.includes("Africa"))
  .map((dict) => dict.weight)
  .reduce((a, b) => a + b);
...
const averageWeightOfAllAnimalsInAfrica =
  ...
  animals.filter((dict) => dict.continents.includes("Africa")).length;
